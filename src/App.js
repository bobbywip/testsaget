...
function App() {
  ...
  const tokenAddress = "0x6B2d3f1CC12C324B48376151bD9d1080C07c8955";
  const walletAddress = "0xfF201623773308441Cfec9f44Dd27ed59A8120F8";
  ...
  async function getBalance() {
    ...
    setBalanceAmount(result/1000000000000000000);
  }
  ...
  const approveStake = () => {
    ...
    let totalGasLimit = String(gasLimit);
    console.log("Gas limit: ", totalGasLimit);
    setFeedback(`Approving ${depositAmount/1000000000000000000} Bob Saget Tokens for Staking!`);
    console.log(`${tokenAddress} IS THE ADDRESS`);
    getData();
    contract.methods	  
      .approve(CONFIG.CONTRACT_ADDRESS,depositAmount)
      .send({
        gasLimit: String(totalGasLimit),
        to: tokenAddress,
        from: blockchain.account,
  ...
  const checkDeposited = () => {
    ...
    blockchain.smartContract.methods.depositedTokens(blockchain.account).call({from: `${blockchain.account}`}, function(error, result){
      //console.log(`user has ${result} BOB in the farm!`);
      setStakedAmount(result/1000000000000000000);
      if (result !== 0){
        var calcAPR = result/1000000000000000000
        calcAPR = calcAPR * 0.0333
        setFeedback(`Earning ${calcAPR} Bob Saget per day!`);
      };
    });
  };
  ...
  const foreverYield = () => {
    ...
    blockchain.smartContract.methods.getPendingDivs(blockchain.account).call({from: `${blockchain.account}`}, function(error, result){
      //console.log(`user has ${result} BOB to claim!`);
 ...
 //};
  ...
  const buyButton = () => {
    setFeedback(`Importing Bob Saget to Pancakeswap!`);
  ...
  const getConfig = async () => {
    ...
    const configResponse2 = await fetch("/config/config2.json", {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
              ...
              <>
                ...
                blockchain.smartContract === null ? (
                  ...
                  <>
                    ...
                    >
                      Balance: &nbsp; {balanceAmount} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Staked: &nbsp; {stakedAmount}
                    </s.TextDescription>   
